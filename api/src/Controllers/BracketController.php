...
class BracketController
{
	...
	public static function createBracket(Request $req, Response $res)
	{
		...
		if(!isset($body['depth']) || !is_numeric($body['depth'])) throw new HttpBadRequestException($req);
	...
	public static function getBracket(Request $req, Response $res, $args) {
		$bracket = Bracket::getBracket(intval($args['bracket'])) or throw new HttpNotFoundException($req, 'Bracket not found');
	...
	public static function updateBracketSlot(Request $req, Response $res, $args) {
		$slot = Bracket::getSlot(intval($args['bracketSlot'])) or throw new HttpNotFoundException($req, 'Bracket slot not found');

		$body = $req->getParsedBody();
		if(isset($body['match']) && is_string($body['match']))
		{
			$slot->match = MMatch::fromUrl($body['match'])?->id or throw new HttpNotFoundException($req, 'Referenced match not found');
		}
